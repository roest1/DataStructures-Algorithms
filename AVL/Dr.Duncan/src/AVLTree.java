...
public class AVLTree<E extends Comparable<E>> implements AVLTreeAPI<E> {
    ...
    public boolean isFibonacci() {
        if (root == null || height(root.left) == count - 1 || height(root.right) == count - 2 || count == 1) 
            return true;
        else 
            return false;
    }
    ...
    private int fullCount(Node node) {
        ...
        int fullnodes = 0;
        if (node.left != null && node.right != null) fullnodes++;
        fullnodes += fullCount(node.left) + fullCount(node.right);
        return fullnodes;
    }
